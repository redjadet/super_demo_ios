...
| Flutter add-to-app | iPhone + iPad lanes: `subosito/flutter-action` → `./tool/prepare_flutter_embed.sh` → `SUPERDEMO_REQUIRE_FLUTTER_EMBED=1`; Mac lane skips Flutter link (sdk-filtered) | Same prepare on macOS before iOS Simulator run | — | Frameworks not committed; see [`flutter-add-to-app.md`](flutter-add-to-app.md) |
